...
const int N = 109*109*2 + 2, M = 10*N;
...
void init(){
    ...
    RD(h, w, a, b), n = h*w*2; s = n++, t = n++; m = 2;
    fill(hd, hd+n, 0);
    ...
    REP(i, h) {
        REP(j, w) if (Map[i][j] == '0') {
            if (i != h-1 && Map[i+1][j] == '0') aee(v(i,j,0), v(i+1,j,0), 1);
            if (j != w-1 && Map[i][j+1] == '0') aee(v(i,j,1), v(i,j+1,1), 1);
            aee(v(i,j,0), v(i,j,1), 1);
        }
    }
    REP(i, a) {
        int y; RD(y); --y;
