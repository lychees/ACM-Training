...
struct cmp{
    bool operator()(cPo a, cPo b) const{
        return a.q() < b.q() || det(a, b) > 0;
    }
};

map<Po, int, cmp> H;
Po p[N], q[N]; int ans[N];
int n, m;

int f(Po d, bool r = 0){
    d = Po(-d.y, d.x); if (d.q()) d = -d;
    if (!CTN(H, d)) return 0;
    return H[d];
}
...
int main(){
    ...
    while (cin >> n >> m) {
        REP(i, n) p[i].in();
        REP(i, m) {
            q[i].in(); ans[i] = 0; H.clear();
            REP(j, n) {
                Po d = p[j]-q[i]; if (d.q()) d = -d;
                ans[i] += f(d, 1); ++H[d];
            }
        }
        REP(i, n) {
            H.clear(); REP(j, n) if (i != j) {
                Po d = p[j]-p[i]; if (d.q()) d = -d;
                ++H[d];
            }
            REP(j, m) {
                Po d = q[j]-p[i];
