...
int main()
{
	...
	while (scanf("%d%d",&n,&m)!=EOF){
		...
		rep(s,n){
			...
			rep(i,1<<n) if (c[i]<n-n/2) rep(j,n) if ((z=f[s][i][j])<1<<28) for (x=G^i;x;x-=x&-x)
				{int k=e[x&-x]; if (z+d[j][k]<f[s][i|1<<k][k]) f[s][i|1<<k][k]=z+d[j][k];}
		}
		rep(i,1<<n) if (c[i]*2==n?i<(G^i):c[i]==n/2){
			...
			rep(k,L1){	x1=a[k],z1=d[n+1][x1],z2=d[n][x1];
				rep(k,L1){	y1=a[k],z=f[x1][i][y1];
					rep(k,L2){	x2=b[k],z+=d[y1][x2];
						rep(k,L2){	y2=b[k],z+=f[x2][j][y2];
