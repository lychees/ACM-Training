...
set<pr> a[N],b[N]; seq f1,g1,f2,g2; int Case,n,m,R,C,x,y,ll,rr,c[N*4]; long long ans;
int query(tree)
{
	if (r<ll || l>rr) return 0; if (ll<=l && r<=rr) return c[t];
	M; return query(left)+query(right);
}
int findMin(tree){if (l==r) return l; M; return query(left)?findMin(left):findMin(right);}
void modify(tree,int x,int y)
{if (l==r) c[t]+=y; else {M; x<=mid?modify(left,x,y):modify(right,x,y); c[t]=c[t*2]+c[t*2+1];}}
...
void work(seq &f,seq &g)
{
	...
	rep(i,n){
		while (j<m && f[j].X<=g[i].X) modify(1,1,C,f[j].Y.X,f[j].Y.Y),++j;
		ll=g[i].Y.X,rr=g[i].Y.Y; int res=query(1,1,C); ans+=res;
		if (g[i].X<x && res) x=g[i].X,y=findMin(1,1,C);
		}
	while (j<m) modify(1,1,C,f[j].Y.X,f[j].Y.Y),++j;
}
int main()
{
