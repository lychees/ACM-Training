...
int f[616666][20],c[1<<20],d[25][25],p[1<<20],q[616666],Case,L,n,m,x,y,z,G,ans;
inline int min(int a,int b){return a<b?a:b;}
int main()
{
	freopen("bustour.in","r",stdin);
	freopen("bustour.out","w",stdout);
	rep(i,1<<20) c[i]=i?c[i-(i&-i)]+1:0;
	while (scanf("%d%d",&n,&m)!=EOF){
		...
		rep(i,L) if (c[q[i]]<n-n/2) rep(j,n) if (q[i]>>j&1) rep(k,n) if (!(q[i]>>k&1))
			if (f[i][j]+d[j][k]<f[p[q[i]|1<<k]][k]) f[p[q[i]|1<<k]][k]=f[i][j]+d[j][k];
