...
int Case,n,e,next[M],ed[M],son[N],q[N][N],g[N],id[N]; double R,x,ans,v[N],w[N];
struct node{
	double x,y; node(){}
...
int main()
{
	while (scanf("%lf%d",&R,&n),ans=0,n*=2){
		e=0,--n; rep(i,n*2+1) son[i]=0;
		rep(i,n){
			scanf("%lf",&x);
		...
		rep(i,n){
			...
			rep(j,n+1) if (i&1){
				double L=fabs(b[i].x-a[j].x); if (L>R+eps) continue; L=sqrt(R*R-L*L)*((j>i)==(d[i].y>0)?-1:1);
				w[t]=(node(b[i].x,a[j].y+L)-b[i])/d[i],g[t]=n+j; if (w[t]>-eps && w[t]<1+eps) t++;
			}else{
				double L=fabs(b[i].y-a[j].y); if (L>R+eps) continue; L=sqrt(R*R-L*L)*(j>i?-1:1);
				w[t]=(node(a[j].x+L,b[i].y)-b[i])/d[i],g[t]=n+j; if (w[t]>-eps && w[t]<1+eps) t++;
			}
			rep(j,t){id[j]=j; if (w[j]>pi) w[j]-=2*pi; if (w[j]<-pi) w[j]+=2*pi;}
			sort(id,id+t,cmp); rep(j,t) add(i,g[id[j]],w[id[j]]);
//			printf("i=%d	",i); rep(j,t) printf("%d ",g[id[j]]); puts("");
		}
		rep(i,n+1){
			int t=0;
			rep(j,n) if (j&1){
				double L=b[j].x-a[i].x; if (fabs(L)>R+eps) continue;
				w[t]=pi/2+acos(L/R)*((j<i)==(d[i].y>0)?-1:1),g[t]=j; if (q[j][n+i]) t++;
			}else{
				double L=b[j].y-a[i].y; if (fabs(L)>R+eps) continue;
...
//			printf("i=%d	",n+i); rep(j,t) printf("%.2lf ",w[id[j]]); puts("");
		}
		int S=0,T,p=q[0][n];
		while (S!=2*n){
			x=v[p]; while (v[p]-x<eps) p=next[p];
			T=ed[p],ans+=(v[p]-x)*(S<n?d[S].len():R),p=q[T][S],S=T;
		}
