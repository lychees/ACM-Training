...
const double pi=acos(-1.),eps=1e-6; int n; double r,ans;
struct node{
	double x,y; node(){}
...
bool get(double a,double b,double c,node &C,node &D)
{
	...
	if (delta<eps) return 0; delta=sqrt(delta);
...
double work(node A,node B)
{
	...
	if (!solve(A,B,C)) return r*r*(angel(B)-angel(A))/2;
...
int main()
{
	...
	while (scanf("%d%lf",&n,&r)!=EOF){
		...
		rep(i,n) ans+=work(a[i],a[i+1]); printf("%.9lf\n",ans);
		}
	return 0;
}
