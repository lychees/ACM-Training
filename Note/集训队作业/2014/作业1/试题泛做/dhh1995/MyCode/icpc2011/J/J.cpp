...
const int N=1000005;
int Case,n=N-5,m,ans,f[8][N],a[1005],A[1005];
int main()
{
	...
	rep(i,m) if (A[i]>4) for (int k=6;k;--k) for (int j=n;j>=A[i];--j)
		if (f[k][j]>N && f[k-1][j-A[i]]<=N) f[k][j]=A[i];
	while (scanf("%d",&n),n){
		printf("Case %d:",++Case);
		int k; for (k=0;k<=6;++k) if (f[k][n]<=N) break;
		if (k>6) puts(" impossible");
		else{
			int last=n+1,res=0,x=0;
			for (;res=0,k;--k){
				rep(i,m) if (a[i]>4 && a[i]<=n && a[i]<last && f[k-1][n-a[i]]<a[i])
