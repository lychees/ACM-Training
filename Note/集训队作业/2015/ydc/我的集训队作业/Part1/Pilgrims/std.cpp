...
#define maxm maxn*2
using namespace std;
int n,m,size[maxn],root,dep[maxn],father[maxn],pos[maxn],col[maxn],maxdep[maxn],cnt[maxn];
...
void DFS(int p)
{
	...
	for(int i=start[p];i;i=next[i])
		if(to[i]!=father[p])
		{
			...
			DFS(to[i]);
			if(maxdep[p]<maxdep[to[i]])
				maxdep[p]=maxdep[to[i]],cnt[p]=cnt[to[i]];
			else if(maxdep[p]==maxdep[to[i]]&&maxdep[p])
				cnt[p]+=cnt[to[i]];
			size[p]+=size[to[i]];
		}
}
