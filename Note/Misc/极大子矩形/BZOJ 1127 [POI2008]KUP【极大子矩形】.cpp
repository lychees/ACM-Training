...
int A[N][N], S[N][N], h[N], l[N], r[N];
int k, n;

int Sub(int x0, int y0, int x1, int y1){
...
void solve(){
    REP_2_1(i, j, n, n) if (k <= A[i][j] && A[i][j] <= 2*k){
        printf("%d %d %d %d\n", j, i, j, i);
        return;
    }
    ...
    RST(h); h[0] = h[n+1] = 0; REP_1(i, n){ // 2>k
        REP_1(j, n) h[j] = A[i][j] < k ? h[j]+1 : 0; gao();

        REP_1(j, n) if (h[j] && Sub(i-h[j]+1, l[j]+1, i, r[j]) >= k){

            int x0 = i-h[j]+1, x1 = i, y0 = l[j]+1, y1 = r[j];

            while (Sub(x0, y0, x1, y1) > 2*k){
                if (x0 == x1) --y1;
                else{
                    if (Sub(x0, y0, x0, y1) < Sub(x1, y0, x1, y1)) ++x0;
                    else --x1;
                }
            }
...
int main(){
    ...
    RD(k, n); REP_2_1(i, j, n, n) RD(A[i][j]);
    solve();
}
