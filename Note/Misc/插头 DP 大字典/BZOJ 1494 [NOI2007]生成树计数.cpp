...
int encode(){
    FLC(bb, -1); int n = 1; REP(i, n) bb[i] = i; int s = 0;
    DWN_1(i, m, 1){
        if (!~bb[b[i]]) bb[b[i]] = n++;
        s <<= _M; s |= bb[b[i]];
    }
...
const int Prime = 9979, MaxSize = 1 << 18; int d;
struct hashTable{
...
int Hid[1<<18]; MT::matrix A, B;
bool flag; // dfs for matrix build?

void dfs(int i = 0){
    if (i == m){
        if (flag){
            if (ck()) ++B.d[ii][Hid[encode()]];
        }
        else push();
    }
    else{
...
void init(){
    ...
    swap(src, des); MT::n = H[src].sz; A.init(); REP_N(ii, H[src].sz){
        //H[src].display(ii);
        Hid[H[src].state[ii]] = ii;
        A.d[0][ii] = H[src].key[ii];
    }
    
    flag = 1; B.init(); REP_N(ii, H[src].sz){
        decode(); b[m] = m+1; dfs();
    }
}
