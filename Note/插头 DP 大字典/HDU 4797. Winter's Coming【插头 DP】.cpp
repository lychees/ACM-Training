...
const int N = 25, M = 15, _M = 4, _UM = 15;
char A[N][M];
int n, m;
...
const int Prime = 9979, MaxSize = 1 << 18;
int d;
struct hashTable{
    LL state[MaxSize]; int key[MaxSize];
    int nxt[MaxSize], hd[Prime], sz;
    void clear(){
        sz = 0;
        FLC(hd, -1);
    }
...
char Aij; void push(int c = 1, int dd = 0){
    if (Aij == 'W' && !c) return;
    if (Aij == 'L' && c) return;
    d += dd;
    H[des][c].push();
...
void solve(){
    ...
    src = 0; des = 1; REP(c, 2) H[des][c].clear();

    REP(j, m) if (isdigit(A[0][j])){
        RST(b); b[j+1] = 1; d = 0; Aij = '0';
        push();
    }
    ...
    REP(i, n){
        REP(j, m){
            ...
            Aij = A[i][j]; swap(src, des); REP(c, 2) H[des][c].clear();

            /*cout << i << " " << j << endl;
            cout << " " << H[src][0].sz << endl;
            cout << " " << H[src][1].sz << endl;*/

            bool dn = i == n-1 || isdigit(A[i+1][j]);
            bool rt = j != m-1 && isdigit(A[i][j+1]);

            //cout << dn << endl;

            REP(c, 2) REP(ii, H[src][c].sz){

                decode(H[src][c].state[ii]); d = H[src][c].key[ii];
                int lt = b[j], up = b[j+1];
        ...
        swap(src, des); REP(c, 2) H[src][c].roll(), H[des][c].clear();
    ...
    int z = INF; REP(ii, H[des][1].sz){
        decode(H[des][1].state[ii]); d = H[des][1].key[ii];
        int cnt = 0; REP(i, m+1) if (b[i]) ++cnt;
        if (cnt == 1) checkMin(z, d);
    }
