...
struct Node{vector<Point>st[2];}a[4*N];
...
void update(int rot,int lt,int rt,int q,int x,int y)
{
	if(lt==rt){
            // assert(y >= 0);
            a[rot].st[0].push_back((Point){x,y});
            a[rot].st[1].push_back((Point){x,-y});
            return;
    }
	int mid=(lt+rt)>>1;
