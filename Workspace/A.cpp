...
int rol[N][2]={},col[N][2]={};
map<PII,int> h;
int r0k=0,c0k=0,rkk=0,ckk=0;
int cnt[2]={};

int prec, c;
void gao1(int i, int j) {
    r0k-=((rol[i][0]==0)!=(rol[i][1]==0));
    rkk-=((rol[i][0]>0)&&(rol[i][1]>0));
    --rol[i][prec^(j&1)];
    r0k+=((rol[i][0]==0)!=(rol[i][1]==0));
    rkk+=((rol[i][0]>0)&&(rol[i][1]>0));

    c0k-=((col[j][0]==0)!=(col[j][1]==0));
    ckk-=((col[j][0]>0)&&(col[j][1]>0));
    --col[j][prec^(i&1)];
    c0k+=((col[j][0]==0)!=(col[j][1]==0));
    ckk+=((col[j][0]>0)&&(col[j][1]>0));
    cnt[((i+j)&1)^prec]--;
...
int main()
{
	RDD(n,m,k); p2[0]=1; FOR(i, 1, N) p2[i] = p2[i-1] * 2;

	DO(k){
		int x, y; RDD(x,y,c);
		int i=x,j=y;
		PII p = MP(x,y);
		if(h.count(p)==0) h[p]=-1;
		prec=h[p];
		if(c!=prec) {
			if(prec!=-1) {
                gao1(i,j);
		...
		Int ans= p2[n-r0k] * (rkk==0) + p2[m-c0k] * (ckk==0);
		ans -= (cnt[0]==0)+(cnt[1]==0);
		cout << ans << endl;
		h[p]=c;
	}
