...
const int N = int(2e5) + 9, Z = 26, QN = int(5e5) + 9;

LL ans[QN]; VII query[N]; // pi, id ...
int qn;
...
namespace ACM{
    ...
    char buf[N+N]; int st[N]; int pos[N];
    VI adj[N]; int L[N], R[N];
    int n;
...
#define f trans[p][c]
    ...
    int Insert(int i){
        scanf("%s", buf+st[i]);
        int u = 0;
        for (char *cur=buf+st[i];*cur;++cur){
            int c = *cur - 'a';
            if (!v) v = new_node();
            u = v;
        }
        st[i+1] = st[i] + strlen(buf + st[i]) + 1;
        pos[i] = u;
        return u;
    }
...
int main(){
    ...
    RD(n, qn); Init(); REP(i, qn){
        int l, r, k; RD(l, r, k); --l, --r;

        //cout << l << " " << r << " " << k << endl;
        query[r].push_back(MP(k, i));
        if (l) query[l-1].push_back(MP(-k, i));
    }
    ...
    REP(i, n){
        ...
        int u = 0; for (char*cur=buf+st[i];*cur;++cur){
            int c = *cur - 'a';
            u = trans[u][c];
            BIT::Add(L[u], 1);
        }
