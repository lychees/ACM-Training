...
namespace SAM{
    ...
    const int N = int(2.2e6) + 9, Z = 3;
    int trans[N][Z], fail[N], len[N], tail, tot;
    char str[N/2];
...
#define ff fail[uu]
    ...
    int f[N/2], g[N/2], q[N/2], cz, op, tt;
    ...
    bool check(int x){
        ...
        cz = 1, op = 0; REP_1(i, n){
            ...
            if (~(tt=i-x)){
                while (cz <= op && op[q][g] < g[tt]) --op;
                q[++op] = tt;
            }

            while (cz <= op && q[cz] < ll[i]) ++cz;

            f[i] = max(f[i-1], cz <= op ? cz[q][g] + i : 0);
            g[i] = f[i] - i;
        }
...
int main(){
    ...
    int Q, nn; RD(Q, nn); Init(); while (nn--){
        RS(str); REP_S(cur, str) Ext(*cur - '0');
        if (nn) Ext(Z-1);
    }

    DO(Q){

        n = strlen(RS(str+1)), n0 = ceil(n*0.9 - EPS); Spell();
