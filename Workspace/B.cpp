...
void solve(){
    RD(h, w, n);
    REP(i, n) RD(P[i].fi, P[i].se);
    P[n].fi = h, P[n].se = w;

    REP(i, n+1) --P[i].fi, --P[i].se;

    REP(i, n+1){

        dp[i] = Calc(P[i].fi, P[i].se);

        REP(j, n) if (P[j].fi <= P[i].fi && P[j].se <= P[i].se){
            dp[i] -= dp[j] * Calc(P[i].fi-P[j].fi, P[i].se-P[j].se);
        }
    }
