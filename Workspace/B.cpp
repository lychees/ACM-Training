...
template<class T> inline T& RD(T &x){
    for (++ch;*ch<=32;++ch); for (x=0;'0'<=*ch;ch++) x = x*10+*ch-'0';
...
int Query_Max(int x, int l, int r){
    ...
    if (a <= l && r <= b){
        ...
        rls(xx);
        return max(Query_Max(lc), Query_Max(rc));
    }
}
...
LL Query_Sum(int x, int l, int r){
    ...
    if (a <= l && r <= b){
        ...
        rls(xx);
        return Query_Sum(lc) + Query_Sum(rc);
    }
}
...
int main() {
    ...
    Rush{
        int m; RD(n, m); REP_1(i, n) RD(A[i]);
        Build(root); DO(m){
            int cmd; RD(cmd); RD(a, b);
            if (cmd == 0){
                RD(t); Insert(root);
            }
            else if (cmd == 1){
