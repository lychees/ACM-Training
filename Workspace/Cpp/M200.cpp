...
template<class T> inline T& RDS(T &x){
    char c;while(!d);x=c-'0';
    return x;
}
...
int rota(int n){
    ...
    REP(i, s.size()-1) r.push_back(s[i]);
    n = toInt(r);
    return n;
}
void bfs(int n){
    queue<PII> q;
    q.push(make_pair(n*a, 1));
    is[n*a] = true;
    if(n % 10 && n > 10){
        q.push(make_pair(rota(n), 1));
        is[rota(n)] = true;
    }
    while(!q.empty()){
        PII u = q.front();
        int num = u.first; int stp = u.second;
        if(num == toInt(s)){
            cout << stp;
            return;
        }
        q.pop();
        if(num*a <= N){
            if(toStr(num*a).size() <= s.size()){
                if(!is[num*a]){
                    q.push(make_pair(num*a, stp+1));
                    is[num*a] = true;
                }
            }
        }
        if(num % 10){
            if(!is[rota(num)]){
                q.push(make_pair(rota(num), stp+1));
                is[rota(num)] = true;
            }
        }
    }
