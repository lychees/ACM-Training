...
namespace ACM{
    const int N = int(1e4) + 9, Z = 10, L = int(1e6) + 9;
...
#define vis Q
    ...
    int a[1209]; int F[1209][N][2][2];
    int n;

    void Init(){
        tt = 0, new_node(); n = strlen(RS(buf));
        REP(i, n) a[i] = buf[i] - '0';
        Rush Insert(); Build(); FLC(F, -1);
    }
...
#undef f
#undef c
    int f(int k = 0, int u = 0, bool st = 1, bool b = 1){
        if (cnt[u]) return 0;
        if (k == n){
            return !st;
        }
            ...
            if (z == -1){
                z = 0; if (st) z += f(k+1, 0, 1, 0);

                int up = 10; if (b) up = a[k];
                FOR(c, st, up) INC(z, f(k+1, v, 0, 0));
