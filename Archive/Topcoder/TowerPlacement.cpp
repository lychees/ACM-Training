...
struct scc_graph {
  public:
    ...
    void dfs(int v, vector<int>& low, vector<int>& ord, vector<int>& ids, int& now_ord, vector<int>& visited, csr<edge>& g, int& group_num){
        low[v] = ord[v] = now_ord++;
        visited.push_back(v);
        for (int i = g.start[v]; i < g.start[v + 1]; i++) {
            auto to = g.elist[i].to;
            if (ord[to] == -1) {
                dfs(to, low, ord, ids, now_ord, visited, g, group_num);
    ...
    std::pair<int, std::vector<int>> scc_ids() {
        ...
        for (int i = 0; i < _n; i++) {
            if (ord[i] == -1) dfs(i, low, ord, ids, now_ord, visited, g, group_num);
        }
        for (auto& x : ids) {
            x = group_num - 1 - x;
        }
...
bool ok(int m) {
    ...
    REP(i, m) {
        if (x[i] == -1) continue;
        PII pi = {x[i], y[i]}, pj; if (CTN(H, pi)) continue;

        if (x[i] == 0 || x[i] == R-1) G.add_clause(i, 0, i, 0);
        if (y[i] == 0 || y[i] == C-1) G.add_clause(i, 1, i, 1);
...
#define j H[pj]
        ...
        pj = MP(x[i]-2, y[i]); if (CTN(H, pj)) G.add_clause(i, 0, j, 0);
        pj = MP(x[i]+2, y[i]); if (CTN(H, pj)) G.add_clause(i, 0, j, 0);
        pj = MP(x[i], y[i]-2); if (CTN(H, pj)) G.add_clause(i, 1, j, 1);
        pj = MP(x[i], y[i]+2); if (CTN(H, pj)) G.add_clause(i, 1, j, 1);

        for (int dx=-1;dx<=1;++dx) if (dx) {
            for (int dy=-1;dy<=1;++dy) if (dy) {
                pj = MP(x[i]+dx, y[i]+dy);
                if (CTN(H, pj)) {
...
class TowerPlacement {
	...
	int solve(int R, int C, int seed, int X) {
	    ::R = R; ::C = C; LL state = seed, mask = _U(31);
        REP(i, N) {
