...
LL D[N], fa[N], dp[N], v[N], w[N], limit[N];
VI adj[N]; int out[N], nn;
int n;

namespace Convex_Hull {
#define ml (l+r>>1)
#define mr (ml+1)
    LL query(VP& C, const Po& p) {
        if (C.empty()) return INFF;
        int l = 0, r = SZ(C)-1;
        while (l < r) {
            if (dott(C[mr] - C[ml], p) < 0) l = mr;
            else r = ml;
        }
        return dot(C[l], p);
    }

    void push(VP& C, const Po& p) {
        int n = SZ(C)-1;
        while (n > 0 && dett(C[n-1], C[n], p) < 1) --n, C.pop_back();
        C.PB(p);
    }
}
...
#define rc rx, mr, r
#define root 1, 1, n
    VP T[NN]; int a, b; Po p;

    void Insert(int x, int l, int r) {
        Convex_Hull::push(T[x], p);
        if (l < r) {
            if (a < mr) Insert(lc);
            else Insert(rc);
        }
    ...
    LL Query(int x, int l, int r) {
        ...
        if (a <= l && r <= b) return Convex_Hull::query(T[x], p);
...
void dfs(int u = 1) {
    ...
    if (u != 1) {
        ...
        int l = 0, r = SZ(path)-1;
        while (l < r) {
            if (limit[u] <= D[path[ml]]) r = ml;
            else l = mr;
        }

        a = out[u]; b = out[path[l]]; p = Po(-v[u], 1);
        dp[u] = Query(root) + w[u] + D[u]*v[u];
    }

    a = out[u]; p = Po(D[u], dp[u]);
    Insert(root);
...
int main() {
    ...
    int t; RD(n, t); FOR_1(i, 2, n){
        adj[RD(fa[i])].PB(i);
        RD(D[i], v[i], w[i], limit[i]);
    }

    dfs0(), dfs();

    FOR_1(i, 2, n) printf("%lld\n", dp[i]);
}
