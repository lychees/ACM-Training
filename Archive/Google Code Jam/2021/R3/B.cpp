...
bool bad(int x, int y, int m){
    REP(i,m){
        if(!A[x+m-i-1][y+i])return 0;
        if(!A[x+2*m-i-1][y+m+i])return 0;
        if(A[x+i][y+m+i])return 0;
        if(A[x+i+m][y+i])return 0;
    }
    REP(i,m){
        A[x+m-i-1][y+i]=A[x+2*m-i-1][y+m+i]=0;
        A[x+i][y+m+i]=A[x+i+m][y+i]=1;
    }
    return 1;
}
...
bool findSquare() {
    REP_2(i, j, n, m) {
        for(int d=1;i+d<n&&j+d<m;d+=1) if(bad(i,j,d)) {
            return 1;
        }
    }
    return 0;
