...
LL f(Po p) {
    int l = 0, r = top;
    while (l < r) {
        if (dott(sta[mr] - sta[ml], p) < 0) l = mr;
        else r = ml;
    }
    return l;
}
LL ff(Po p) {
    int l = 0, r = top;
    while (l < r) {
        if (dett(sta[ml], sta[mr], p) > 0) l = mr;
        else r = ml;
    }
    return l+1;
}
...
void dfs(int u = 1) {
    ...
    if (u != 1) {
        ...
        dp[u] = INFF;
        REP(i, top+1) {
            checkMin(dp[u], (LL)dot(p, sta[i]) + w[u] + D[u]*v[u]);
        }
        */
        dp[u] = (LL)dot(p, sta[f(p)]) + w[u] + D[u]*v[u];
    }

    Po p = Po(D[u], dp[u]); int _top = top; top = ff(p);
    Po _sta = sta[top];  sta[top] = p;
