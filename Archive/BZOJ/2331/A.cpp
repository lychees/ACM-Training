...
const int MOD = 20110520; //int(1e9) + 7;
...
const int N = int(1e2) + 9, M = 1 << 19, _M = 2;

int A[N][N]; char buf[N][N];
int n, m;

const int Prime = 9979;
int d; struct hashMap{
    int state[M], key[M]; int sz;
    ...
    void roll(){
        REP(ii, sz) state[ii] <<= 1;
    }
} H[2]; int src, des;


void init(){
    REP(i, n) RS(buf[i]); RST(A);
    if (m > n){
        swap(n, m);
        REP(i, n) REP(j, m) A[i][j] = buf[j][i] == '_';
    }
    else{
        REP_2(i, j, n, m) A[i][j] = buf[i][j] == '_';
    }
}

void solve(){

    src = 0, des = 1; H[des].clear(); d = 1; H[des].push(0);
    ...
    REP(i, n){
        REP(j, m){
            ...
            REP(ii, H[src].sz){
                ...
                int lt = _1(s, j), up = _1(s, j+1), ltt = _1(s, j+m+1), upp = _1(s,j+1+m+1);
                bool dn = A[i+1][j], rt = A[i][j+1];
                s &=~ (_1(j)|_1(j+1)|_1(j+m+1)|_1(j+1+m+1));
                ...
                if (lt && up){
                    ...
                    if (lt){
                        if (ltt){
                            if (dn) H[des].push(s|_1(j));
                            if (rt) H[des].push(s|_1(j+1)|_1(j+1+m+1));
                        }
                        else{
                            if (rt) H[des].push(s|_1(j+1));
                            H[des].push(s);
                        }
                    }
                    else{
                        if (upp){
                            if (rt) H[des].push(s|_1(j+1));
                            if (dn) H[des].push(s|_1(j)|_1(j+m+1));
                        }
                        else{
                            if (dn) H[des].push(s|_1(j));
                            H[des].push(s);
                    ...
                    if (dn && rt) H[des].push(s | _1(j) | _1(j+1));
                    if (dn) H[des].push(s | _1(j) | _1(j+m+1));
                    if (rt) H[des].push(s | _1(j+1) | _1(j+1+m+1));
                }
            }
