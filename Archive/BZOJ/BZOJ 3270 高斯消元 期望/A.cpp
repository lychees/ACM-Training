...
namespace Gauss {
    ...
    void gauss() {
        ...
        REP(i, n) {
            ...
            if (ii != i) {
                REP(j, n+1) swap(A[i][j], A[ii][j]);
            }

            DB &r = A[i][i]; assert(sgn(r));
            FOR(j, i+1, n+1) A[i][j] /= r;
            r = 1;

            REP(j, n) {
                if (i == j) continue;
                DB &r = A[j][i]; if (!sgn(r)) continue;
                FOR(k, i+1, n+1) A[j][k] -= A[i][k] * r;
                r = 0;
            }
        }
    }
    ...
    void gao() {
        ...
        REP(i, ::n) REP(j, ::n) {
            int u = g(i, j);
            A[u][u] = 1;
            ECH(it, adj[i]) ECH(jt, adj[j]) {
                int ii = *it, jj = *jt; if (ii == jj) continue;
                DB pi = ii==i?stay[ii]:(1-stay[ii])/(SZ(adj[ii])-1);
                DB pj = jj==j?stay[jj]:(1-stay[jj])/(SZ(adj[jj])-1);
                int v = g(ii, jj);
                A[u][v] -= pi*pj;
            }
        }
