...
void cdq(int l, int r){
    ...
    if (l + 1 == r) {
        ...
        cdq(l, ml);
        static Po P[N], C[N]; int pn = 0, cn = -1;
        FOR(i, l, ml) P[pn++] = Po(a(i), b(i)); sort(P, P+pn);

        REP(i, pn){
            while (cn > 0 && dett(C[cn-1], C[cn], P[i]) >= 0) --cn;
            C[++cn] = P[i];
        }
        ...
        FOR(i, ml, r) {
            ...
            int l = 0, r = cn;
            while (l < r) {
                if (dot(C[mr]-C[ml], p) > 0) l = mr;
                else r = ml;
            }
            checkMax(f[i], dot(C[l], p));
        }
