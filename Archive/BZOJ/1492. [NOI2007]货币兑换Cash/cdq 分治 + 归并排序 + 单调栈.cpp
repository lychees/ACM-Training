...
void cdq(int l, int r, vector<Po>& C){
    ...
    if (l + 1 == r) {
        ...
        static int QQ[N]; int ll = l, rr = ml; FOR(i, l, r) QQ[Q[i] < ml ? ll++ : rr++] = Q[i];
        memcpy(Q+l, QQ+l, sizeof(int) * (r-l));

        cdq(l, ml, C);

        ll = 0; int cn = SZ(C)-1; FOR(qi, ml, r) {
            int i = Q[qi]; Po p = Po(A[i], B[i]);
            while (ll < cn && dot(C[ll], p) < dot(C[ll+1], p)) ++ll;
            checkMax(f[i], dot(C[ll], p));
        }

        vector<Po> Cr; cdq(ml, r, Cr);

        vector<Po> P; cn = SZ(C) + SZ(Cr); C.PB(Po(OO, OO)); Cr.PB(Po(OO, OO));
        ll = 0, rr = 0; DO(cn) P.PB(C[ll] < Cr[rr] ? C[ll++] : Cr[rr++]);

        C.resize(SZ(P)); cn = -1; for (auto p: P){
            while (cn > 0 && dett(C[cn-1], C[cn], p) >= 0) --cn;
            C[++cn] = p;
        }
        C.resize(cn+1);
    }
...
int main(){
    ...
    RD(n); RF(f[0]); REP(i, n) RF(A[i], B[i], R[i]), Q[i] = i;
