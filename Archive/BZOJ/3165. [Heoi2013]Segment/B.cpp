...
struct Line {
    int x0,x1,id; static int x;
    DB y0,y1;
    Line(){
    }
    Line(int x0,int x1,DB y0,DB y1,int id):x0(x0),x1(x1),y0(y0),y1(y1),id(id) {
    }
    DB k() const {
        return (y1-y0)/(x1-x0);
    }
    ...
    bool operator<(const Line& r) const {
        return sgn(y(x),r.y(x)) < 0 || sgn(y(x),r.y(x)) == 0 && id > r.id;
    }
};
...
namespace Segtree {
    ...
    void _insert(int x, int l, int r, Line t) {
    ...
    Line Query(int x, int l, int r) {
        if (l == r) return T[x];
        return max(T[x], Line::x < mr ? Query(lc) : Query(rc));
    }
}
...
int main(){
    ...
    int i = 0; Rush {
        if (!RD()){
            Line::x = (RD()+last_ans-1)%N+1;
            OT(Segtree::Query(root).id);
        }
        else{
            int x0 = (RD()+last_ans-1)%N+1;
            int y0 = (RD()+last_ans-1)%int(1e9+1);
            int x1 = (RD()+last_ans-1)%N+1;
            int y1 = (RD()+last_ans-1)%int(1e9+1);
            if (x0>x1) swap(x0,x1),swap(y0,y1);
            Segtree::t = Line(x0,x1,y0,y1,++i);
            Segtree::Insert(root);
        }
    }
}
