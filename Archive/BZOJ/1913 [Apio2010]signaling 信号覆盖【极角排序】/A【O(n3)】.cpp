...
LL binom(int n, int m){ // m > n 时会算出 0, 注意精度问题。
...
DB solve(){
    ...
    LL z = 0; REP(i, n) FOR(j, i+1, n){
        int cnt = 0; REP(k, n) if (k != i && k != j){
            if (dett(P[i], P[j], P[k]) == 1) ++cnt;
        }
        z += binom(cnt, 2) + binom(n-2-cnt, 2);
    }
    z -= binom(n, 4)*2;
    return (DB)z/binom(n,3) + 3;
}
