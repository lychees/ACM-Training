...
VI adj[N]; int dfn[N], low[N], par[N], dep[N];
int f[N], w[N]; int n, m, nn, z;

// dp on circle
int c[N], q[N], cz, op; // 1.5n careful!
#define ff(x) (c[x]-x)
void dpc(int rt, int ed){
    int n = dep[ed]-dep[rt]+1; c[0] = f[rt];
    for (int i=1,u=ed;u!=rt;u=par[u],++i) c[i] = f[u];
    int cn = n+n/2; FOR(i, n, cn) c[i] = c[i-n];
    cz = 0, op = -1, q[++op] = 0; FOR(i, 1, cn){
        while (cz <= op && i-q[cz]>n/2) ++cz;
        checkMax(z, c[i]+i+ff(q[cz]));
        while (cz <= op && ff(q[op])<=ff(i)) --op;
        q[++op] = i;
    }
    FOR(i, 1, n) checkMax(f[rt], c[i] + min(i, n-i));
}
