...
set<pair<int, int> > Ban2;
set<int> Ban; vector<Po> P;

void add(vector<Po>& P, int a, int b){
    if (CTN(Ban2, MP(a, b))) return;
    if (L[a].a * L[b].b == L[b].a * L[a].b) return;
    Line l1(L[a].a, L[a].b, -L[a].c);
    Line l2(L[b].a, L[b].b, -L[b].c);
    P.PB(l1 * l2);
}
...
int main(){
    ...
    RD(n, m); REP(i, n) L[i].in();
    
    REP(i, m){
        int a, b; RD(a, b); --a, --b;
        Ban.insert(a); Ban.insert(b);
        Ban2.insert(MP(a, b));
        Ban2.insert(MP(b, a));
    }
    
    REP(i, n) if (!CTN(Ban, i)) o.PB(i);
    
    if (!o.empty()){
        sort(ALL(o), cmp); o.PB(o[0]);
    }
    
    REP(i, o.size()-1) add(P, o[i], o[i+1]);
    ECH(it, Ban){
        vector<Po> t; REP(i, n) add(t, *it, i);
        if (!t.empty()){
            SRT(t); P.PB(t[0]);
            if (t.size()>=2) P.PB(t.back());
        }
    }
    printf("%.9f\n", getPeri(getCH(P)));
}
