...
void ins(int x = 1, int xx = 1, int dep = 0){
    if (dep == m){
        leaf[x][xx] += cur;
    }
    else if (dep >= m/2){
        if (buf[dep] == '0') ins(x, xx<<1, dep+1), ins(x, (xx<<1)|1, dep+1);
        else ins(x, (xx<<1)|1, dep+1);
    }
    else{
        if (buf[dep] == '0') ins(lx, 1, dep+1);
        else ins(rx, 1, dep+1);
    }
}

Int qry(int x = 1, int dep = 0){
    if (dep == m/2){
        int xx = 1; FOR(d, dep, m){
            xx <<= 1; if (buf[d]=='1') xx |= 1;
        }
        return leaf[x][xx];
    }
    else{
        return qry(lx, dep+1) + (buf[dep] == '1' ? qry(rx, dep+1) : Int(0));
    }
}
...
int main(){
    ...
    while(~scanf("%d%d",&n, &m)){
        ...
        REP(i, N) REP(j, N) leaf[i][j] = 0;
        
        Int z = 0; FLC(buf, '0'); cur = 1; ins();
