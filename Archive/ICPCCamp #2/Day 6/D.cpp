...
int main(){
    ...
    while (cin >> n){
        ...
        REP_1(i, n) RD(a[i]); a[n+1] = n+1;
        
                /*RST(c); c[0][0] = 1; REP(i, n+1) REP(j, i+1){
            FOR_1(ii, i+1, n+1) if (a[i] < a[ii]){
                c[ii][j+1] += c[i][j];
            }
        }*/
        
        RST(c); c[0][0] = 1; REP(len, n+1){
            
            BIT::Init();
            BIT::Add(0, c[len][0]);
            
            REP_1(i, n+1){
                c[len+1][i] = BIT::Sum(a[i]);
                BIT::Add(a[i], c[len][i]);
            }
        }
        ...
        /*RST(f); f[0][0] = 1; REP(i, n){
            REP(j, n) FOR_1(jj, j+1, n) f[i+1][jj] += f[i][j] * binom[n-j][jj-j];
         }*/
        
        RST(f); REP(i, n+1){
            f[n][i] = pow(i, n);
            FOR(j, 0, i) if ((i-j)&1) f[n][i] -= binom[i][j] * pow(j, n);
            else f[n][i] += binom[i][j] * pow(j, n);
        }
