...
namespace SAM{
    ...
    char str[N]; int trans[N][Z], fail[N], len[N], tail, tot;
...
// http://user.qzone.qq.com/251815992/blog/1444314774
...
#define c (*cur - 'a')
    RS(S); RS(T); Init(); REP_S(cur, T) Ext(c);
    
    int u = 0, l = 0, i = 0, z = -INF, ll = -1, rr = -1;
    int n = strlen(S); REP_S(cur, S){
        while (u && !v) l = len[u = f];
        if (u = v) ++l;
        
        if (l && l - max(i-l+1, n-i-1) > z){
            z = l - max(i-l+1, n-i-1);
            ll = i-l+1, rr = i;
        }
        ++i;
