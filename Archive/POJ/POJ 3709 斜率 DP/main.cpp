...
int q[N], cz, op;
     ...
     /*
      ...
      // x = a[j+1]
      // y = dp[j]+x*j-s[j]
        dp[i] = -a[j+1]*i + dp[j] + a[j+1]*j-s[j];
        dp[i] += s[i]
      */

#define kk (-i)
#define x(i) (a[i+1])
#define y(i) (dp[i]-s[i]+i*x(i))
#define eval(i) (kk*x(i)+y(i))


// = dp[j] + (s[i] - s[j]) - (a[j+1] * (i - j)));
// -i*a[j+1] + (dp[j]-s[j] - a[j+1]  )
// b = kx + y
// di =  -i aj+1   dj - sj + j aj+1
// di =
...
int main() {
    ...
    Rush {
        RD(n, k); REP_1(i, n) s[i] = s[i-1] + RD(a[i]);
        ...
        REP_1(i, n) {
            dp[i] = INF;
            REP(j, i - k + 1) {
                checkMin(dp[i], dp[j] + (s[i] - s[j]) - (a[j+1] * (i - j)));
            }
        }
        */
        ...
        cz = 0, op = 0; q[cz] = 0; REP_1(i, n){
            if (i-k >= k) {
                while (cz < op && dett(q[op-1], q[op], i-k) <= 0) --op;
                q[++op] = i-k;
            }
            while (cz < op && eval(q[cz]) >= eval(q[cz+1])) ++cz;
            dp[i] = cz <= op ? (eval(q[cz]) + s[i]) : INF;
            //cout << i << ": " << dp[i] << " " << q[cz] << " " << eval(0) <<endl;
        }
