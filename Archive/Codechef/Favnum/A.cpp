...
namespace ACM{
    const int N = int(1200) + 9, Z = 10, bufN = int(1e6) + 9;
...
#undef c
    ...
    int a[109]; LL F[2][2][109][N][2];
    int n; LL L, R;

    LL f(int k = n-1, int u = 0, bool ok = 0, bool st = 1, bool b = 1){
        ...
        if (k == -1){
            ...
            if (z == -1){
                z = 0; if (st) z += f(k-1, 0, 0, 1, 0);

                int up = 10; if (b) up = a[k];
                FOR(c, st, up) z += f(k-1, v, ok, 0, 0);
    ...
    LL ff(LL x){
        ...
        REP(c1, 2) REP(c2, 2) if (c1 || c2) FLC(F[c1][c2], -1);
        return f();
    }
    ...
    void Init(){
        ...
        FLC(F, -1); LL k; cin >> k;
        tt = 0, new_node(); Rush Insert();
        Build(); FLC(F, -1); k += ff(L-1);
