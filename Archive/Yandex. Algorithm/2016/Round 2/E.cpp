...
VI adj[N]; int a[N], o[N], sz[N], L[N], R[N], d0[N], d1[N], nn;
LL down[N], up[N];
...
void dfs3(int u = 0, int p = -1){
    ECH(it, adj[u]) if (v != p){
        up[v] = up[u] + (down[u]-down[v]-d1[v]) + (C[a[v]] - d0[v]);
        dfs3(v, u);
    }
}
...
int main(){
   ...
   // freopen("out.txt", "w", stdout);
    ...
    REP(i, n) o[i] = i;
    sort(o, o+n, [](const int &l, const int &r){
         return a[l] < a[r] || a[l] == a[r] && sz[l] > sz[r];
    });

    REP(i, n){
        int u = o[i];
        d0[u] = Sum(L[u], R[u]);
        ECH(it, adj[u]) if (sz[v] < sz[u]) d1[v] = Sum(L[v], R[v]);
        Add(L[u], 1);
    }

    RST(C); REP(i, n) ++C[a[i]+1];
    REP(i, n) C[i+1] += C[i];
    dfs2(); up[0] = 0; dfs3();

    int z = 0; FOR(i, 1, n) if (down[i] + up[i] < down[z] + up[z]) z = i;
    cout << z+1  << " " << down[z] + up[z] << endl;
}
