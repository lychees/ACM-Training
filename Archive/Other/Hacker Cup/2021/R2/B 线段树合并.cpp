...
#define rc rx, mr, r
    const int NN = 100*N;
    int c[2][NN]; int ss[NN]; bool bad[NN]; int tot;
    int pp;
    int new_node() {
        int x = ++tot;
        lx = 0; rx = 0; ss[x] = 0; bad[x] = 0;
        return x;
    }
    void upd(int x) {
        bad[x] = bad[lx] | bad[rx];
    ...
    void Insert(int &x, int l = 1, int r = n) {
        if (!x) x = new_node();
        if (l == r) {
            ss[x] += 1;
            bad[x] = (ss[x] < cnt[l]);

            //cout << " ### " << x << " " << l << " " << ss[x] << " " << cnt[l] << endl;
...
int main(){
    ...
    Rush {
        ...
        REP_1(i, n) cnt[i] = 0, cur[i] = 0, rt[i] = 0, adj[i].clear();
