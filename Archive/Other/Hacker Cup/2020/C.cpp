...
int main(){
    ...
    Rush{
        printf("Case #%d: ", ++Case);
        RD(n); REP(i, n) RDD(a[i].fi), RD(a[i].se);
        sort(a, a+n);
        
        l.clear(), r.clear(); int z = 0;
        
        for (int i=0,j;i<n;i=j){
            j = i+1; while (j<n && a[j].fi == a[i].fi) ++j;
            int x = a[i].fi;
        ...
        for (int i=n-1,j;i>=0;i=j){
            j = i-1; while (j>=0 && a[j].fi == a[i].fi) --j;
            int x = a[i].fi;
