...
int main(){
    ...
    Rush{
        ...
        H.clear(); Rush ++H[getPo()];
        P.clear(); ECH(it, H) P.PB(it->fi);

        z = 0;
        REP(i, SZ(P)){
            int c = Int(pow(2, H[P[i]])) - 1; z += c - H[P[i]];
            VP PP; REP(j, i) PP.PB(P[j] - P[i]); SRT(PP, cmp);
            int l = 0; while (l < PP.size()){
                int cc = H[PP[l]+P[i]] , r = l+1; while (r < PP.size() && !dett(PP[l], PP[r])) cc += H[PP[r]+P[i]], ++r;
                z += (Int(pow(2, cc)) - 1) * c; l = r;
            }
        }
        printf("%d\n", int(z));
    }
