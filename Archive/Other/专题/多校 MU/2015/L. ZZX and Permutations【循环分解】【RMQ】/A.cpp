...
namespace ST{
    ...
    void Modify(int x, int l, int r){
        if (l == r) T[x] = b;
            ...
            if (a < mr) Modify(lc); else Modify(rc);
            upd(x);
        }
    }
...
int getL(int p){
    int l = *--C.upper_bound(p);
    return Query(l+1, p);
}

int getR(int p){
    return CTN(C, p+1) ? 0 : A[p+1];
}
...
int main(){
    ...
    Rush{
        REP_1_C(i, RD(n)) pos[RD(A[i])] = i;

        Build(rt); C.clear(); C.insert(0); C.insert(n+1); RST(B); REP_1(i, n) if (!B[i]){

            int p = pos[i], l = getL(p), r = getR(p);
