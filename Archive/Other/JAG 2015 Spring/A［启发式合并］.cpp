...
int w[N], h[N], dep[N]; map<int, int> f[2][N];
VI adj[N]; int n; LL z;

#define v (*it)

int merge(int a, int b, int off){
    
    if (max(f[0][a].size(), f[1][a].size()) < max(f[0][b].size(), f[1][b].size())) swap(a, b);
    
    REP(i, 2) for (auto it: f[i][b]) if (CTN(f[1-i][a], off-it.fi))
        z += (LL)it.se * f[1-i][a][off-it.fi];

    REP(i, 2) for (auto it: f[i][b]) f[i][a][it.fi] += it.se;
    return a;
}

void dfs(int u, int p = -1){
    h[u] = u; dep[u] += w[u];
    f[w[u] == 1 ? 0 : 1][u][dep[u]] = 1;
    
    ECH(it, adj[u]) if (v != p){
        dep[v] = dep[u]; dfs(v, u);
        h[u] = merge(h[u], h[v], 2*dep[u]-w[u]);
        
        if (w[u] != 1) f[0][h[u]].erase(dep[u]-w[u]-1);
        else f[1][h[u]].erase(dep[u]-w[u]+1);
    }
}
