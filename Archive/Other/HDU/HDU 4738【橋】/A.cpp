...
#define cSeg const Seg&
    ...
    struct Line{
        ...
        Line&push(DB r){ // Õý ýÓÒ ÖÂÝÐýÏòÀï
    ...
    inline int Seg::sgn(cPo p)const{
        if (dett(p, a, b)) return -1; // ÓÐ ±»áÓÐ¾«¶ÈÎó²î¡£¡£
        if (a == p || b == p) return 0;
        return -dott(p,a,b);
    }
...
void tarjan(int u=0,int p=-1) {
    ...
    ECH(it, adj[u]) {
        int v = *it, w = weight[{min(u, v), max(u, v)}];
        if (v == p) continue;
        if (dfn[v]) {
...
#endif
    while (RD(n, m)) {
        RST(dfn, low); REP(i, n) adj[i].clear(); weight.clear();
        REP(i, m) {
