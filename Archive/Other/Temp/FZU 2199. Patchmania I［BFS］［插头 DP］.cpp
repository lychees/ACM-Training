...
const int N = int(10), M = 1<<18, _M = 3;
char A[N+1][N+1]; int n, m;
int b[N+1], bb[N+1];
...
const int Prime = 9979, MaxSize = M; LL d;
struct hashTable{
...
bool AA[N+1][N+1]; int Dist[N][N], Ways[N][N];

int Qx[N*N], Qy[N*N], cz, op;
int step, step0, ways;
...
void solve(){
    step = INF, step0 = 0, ways = 0;
    
    RD(n, m); RST(AA); cz = 0, op = 1;
    RST(Dist, Ways); REP_2(i, j, n, m){
        RC(A[i][j]); if (A[i][j] == 'X'){
            Qx[0] = i, Qy[0] = j;
            Dist[i][j] = 1;
            Ways[i][j] = 1;
        }
        AA[i][j] = A[i][j] == '#' || A[i][j] == 'O';
        if (A[i][j] == '#') step0 += 1;
    }
    ...
    while (cz < op){
        ...
        int ux = Qx[cz], uy = Qy[cz], du = Dist[ux][uy]; ++cz;
        if (du > step) break;
        ...
        if (A[ux][uy] == '#'){
            ...
            if (H[des].sz == 1){
                step = du;
                INC(ways, pdt(Ways[ux][uy], H[des].key[0]));
            }
            continue;
        }
        ...
        REP(i, 4){
            ...
            if (0 <= vx && vx < n && 0 <= vy && vy < m){
                if (Dist[vx][vy] == du + 1) INC(Ways[vx][vy], Ways[ux][uy]);
                else if (!Dist[vx][vy]){
                    Dist[vx][vy] = du + 1;
                    Ways[vx][vy] = Ways[ux][uy];
                    Qx[op] = vx;
                    Qy[op] = vy;
                    ++op;
