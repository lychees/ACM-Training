...
int r[N]; int n, m;
vector<int> W; int ww[N];
int dp[N][N][N];
// 第 i 层，当前层有 j 个枝杈，需要放置 k 个物品，的最少代价。
...
int f(int i, int j, int k){
    ...
    if (z == -1){
        if (j >= k){
            ...
            z = min(f(i, j-1, k-1)+(i+1)*W[k-1], f(i+1, j*r[i+1], k));
        }
    }
    return z;
}
...
int main(){
    ...
    while (~scanf("%d %d", &n, &m)){
        W.clear(); REP(i, n) W.PB(RD()); REP(i, m) RD(r[i]);
        SRT(W); REP(i, n) ww[i+1] = ww[i] + W[i]; FLC(dp, -1);
        cout << f(0, r[0], n) << endl;
    }
}