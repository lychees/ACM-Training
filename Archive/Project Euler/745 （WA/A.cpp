...
VI P; bitset<PMAX> isP; LL mu[PMAX], mu2[PMAX];
void sieve(){
    mu[1] = 1;
    FOR(i, 2, PMAX){
...
map<LL, LL> s_mu2;
LL S_mu2(LL n) {
    if (n < PMAX) return mu2[n];
    if (!CTN(s_mu2, n)) {
        LL &z = s_mu2[n]; z = S_mu(n);
        for (LL i = 2, j; i <= n; i = j) {
            j = n / (n/i) + 1;
            z -= S_mu2(n/i) * (j-i);
        }
...
int main(){
    ...
    // S =  f  g
    // u = u*u*1
    // S_mu^2(n) = S_mu(n) - sigma S_mu2(n/i)
    ...
    Int z = 0;
    for (LL i=1;i*i<=n;++i) {
        z += Int(i)*i*S_mu2(n/i/i);
    }
    cout << z << endl;

    //int n = 20;
    // 1 2 3 3 4 5 6 6 6 7 8 8 9 10 11 11 12 12 13 13
    //REP_1(i, n) cout << S_mu2(i) << " "; cout << endl;
...
/*
138564905
Process returned 0 (0x0)   execution time : 652.472 s
