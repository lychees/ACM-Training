...
void pnc(int x, int l, int r, int d){
    checkMax(T[x].d, d);  checkMax(T[x].v, d);
}

void rls(int x, int l, int r){
    pnc(lc, T[x].d); pnc(rc, T[x].d);
    T[x].d = 0;
}
...
void Inorder(int x, int l, int r) {
    if (l == r) {
        printf("%d ", T[x].v);
    } else {
        rls(xx);
        Inorder(lc); Inorder(rc);
    }
}
...
int main() {
    ...
    RD(n); REP_1(i, n) RD(A[i]); Build(rt);
    Rush {
        if (RD() == 1) {
            RD(a, b);
            Modify(rt);
