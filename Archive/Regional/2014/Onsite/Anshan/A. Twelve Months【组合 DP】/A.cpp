...
DB f[2][1<<11][N];
DB binom[N][N]; int remain[N], total;
int n, _s, des, src;


void print() {
    REP(s, _1(n-1)){
        DB z = .0; REP(rk, n*4+1) z += f[des][s][rk];
        printf("%.16f\n", z);
    }
}
...
void init(){
    ...
    RD(n); REP(i, n) remain[i] = 4; Rush --remain[RD()-1];
    _s = 0, src = 0, des = 1; RST(f[des]);
    FOR(i, 1, n) if(!remain[i]) _s |= _1(i-1);
    int total = remain[0]; f[des][_s][total] = 1; if (!total) return;

    FOR(i, 1, n) if(remain[i]){

        swap(src, des); RST(f[des]);

#define u f[src][s][rk]
#define v f[des][ss][rkk]
#define p (binom[rkk-1][ii] * binom[total+remain[i]-rkk][remain[i]-ii] / binom[total+remain[i]][total])

        FOR(s, _s, (_s|_1(i-1))+1) if ((s|_s)==s){
            REP(rk, total+1) if (u > 0){
                REP(ii, remain[i]+1){
                    int ss = s; if (ii == remain[i]) ss |= _1(i-1);
                    int rkk = rk + ii;
                    v += u * p;
                }
            }
...
int main() {
    ...
    REP(i, N){
        binom[i][0] = 1.;
        REP_1(j, i) binom[i][j] = binom[i-1][j] + binom[i-1][j-1];
    }
