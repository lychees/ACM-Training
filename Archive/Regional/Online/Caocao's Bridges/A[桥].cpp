...
vector<pair<int, PII> > adj[N]; int dfn[N], low[N];
int n, m; int nn, bn, bb;
#define v (it->fi)
#define w (it->se.fi)
#define eid (it->se.se)
...
int main(){
    ...
    while (~scanf("%d %d", &n, &m) && n){
        ...
        REP(i, m){
            int a, b, w; RD(a, b, w); --a, --b;
            adj[a].PB(MP(b, MP(w, i)));
            adj[b].PB(MP(a, MP(w, i)));
        }
        
        RST(dfn); nn = bn = 0;
        int cpt = 0; bb = INF; REP(i, n) if (!dfn[i]) dfs(i), ++cpt;
        
        if (cpt != 1) bb = 0;
        else{
            if (bb == INF) bb = -1; else if (!bb) bb = 1;
        }
