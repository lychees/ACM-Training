...
const int MOD = 105225319; //int(1e9) + 7;
...
Int binom[N][N], S[N], C[N];
int n, m;

int solve(){

    RD(n, m); if (n == 1) return 0;

    S[0] = 1; REP_1(i, n){
        S[i] = 0;
        REP(j, i+1) S[i] += binom[i][j] * pow(m+1, j*(i-j));
    }
...
int main(){
    ...
    REP(i, N){binom[i][0] = 1; REP(j, i) binom[i][j+1] = binom[i-1][j] + binom[i-1][j+1];}
