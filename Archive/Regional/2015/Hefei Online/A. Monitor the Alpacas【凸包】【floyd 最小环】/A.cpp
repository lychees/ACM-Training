...
bool ok1(){
    if (m == 1){
        ECH(it, B) if (*it == A[0]) return true;
    }
    return false;
}
...
struct Poo{
    Po p; DB arg;
    bool operator <(const Poo& r) const{
        return arg < r.arg;
    }
}; vector<Poo> C;
...
int floyd(){
    ...
    int z = INF; REP(i, n) REP(j, n) if (D[i][j] == 1) checkMin(z, D[j][i]);
    if (z == INF) z = -1; else ++z;
    return z;
}

int solve(){
    A.resize(RD(m)); B.resize(RD(n));
    REP(i, A.size()) A[i].in();
    REP(i, B.size()) B[i].in();
    if (ok1()) return 1;

    A = getCH(A); C.resize(A.size());
    FOR(i, 1, A.size()){
        C[i].p = A[i];
        C[i].arg = (A[i]-A[i-1]).arg();
    }

    sort(C.begin()+1, C.end());
    C[0] = C[C.size()-1]; C[0].arg = -INF;

    FLC(D, 0x3f); REP(i, n) REP(j, n) if (i != j){
        Poo t; t.arg = (B[j]-B[i]).arg();
        Po p = (--upper_bound(ALL(C), t))->p;
        if (~dett(B[i], B[j], p)) D[i][j] = 1;
    }
