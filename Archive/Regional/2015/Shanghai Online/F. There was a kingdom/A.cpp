...
LL solve(){
    ...
    if (m <= 2) return 0;
    int n = C.size()-1; if (m >= n) return getArea(C);

    --m; C.resize(2*n); FOR(i, n, 2*n) C[i] = C[i-n];

    LL z = 0; REP(st, n){
        //DO(10*(n/(m+1))){
        //int st = rand() % n;
        FLC(dp, -1); dp[0][0] = 0; FOR(i, 1, n){
            REP_1(j, m) REP(k, i) if (dp[k][j-1] != -1) checkMax(dp[i][j], dp[k][j-1] + (LL)det(C[st+k], C[st+i]));
            if (dp[i][m] != -1) checkMax(z, dp[i][m] + (LL)det(C[st+i], C[st]));
        }
    }
    return z;
}
int main() {
    ...
    Rush{
        RD(n, m); VP P; DO(n) P.PB(getPo()); C = getCH(P);
        OT(solve());
    }
}
