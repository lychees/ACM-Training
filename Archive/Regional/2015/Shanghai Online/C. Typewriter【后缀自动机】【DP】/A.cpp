...
namespace SAM{
    ...
    char str[N]; int trans[N][Z], fail[N], len[N], time[N], match[N], tail, tot;
    int n;
    ...
    inline int new_node(int u){
        CPY(trans[tot], trans[u]), fail[tot] = fail[u], time[tot] = time[u];
        return tot++;
    }
...
#define ff fail[uu]
    ...
    void Init(){
        tot = 0, new_node(); n = strlen(RS(str));
        REP(i, n) time[Ext(str[i] - 'a')] = i;
...
LL w0[Z], w1, w2; // add, select, copy
LL f[N]; int q[N], cz, op;
...
int main(){
    ...
    Rush{
        ...
        REP(i, Z) RD(w0[i]); RD(w1, w2); w2 *= 2;

#define ff(i) (f[i]-w1*i)

        cz = 0, op = -1;  REP(i, n){
            f[i] = (i ? f[i-1] : 0) + w0[str[i]-'a'];
            while (cz <= op && q[cz] < match[i]) ++cz;
            if (cz <= op) checkMin(f[i], ff(q[cz])+w1*i+w2);
            while (cz <= op && ff(i) < ff(q[op])) --op;
            q[++op] = i;
        }
