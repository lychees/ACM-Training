...
const int N = int(12), M = 1 << 18, _M = 3;

int n, m; bool G[5][5]; //15 25
int b[N+1], bb[N+1];

LL encode(){
    FLC(bb, -1); int n = 0; LL s = 0;

    DWN_1(i, 5, 1) DWN_1(j, 5, 1){
        s <= 1; s |= G[i][j];
    }
...
LL d; struct hashMap{
    ...
    void display(int ii){
        decode(state[ii]); cout << state[ii] << " " << key[ii] << ": "; cout << endl;
...
void trans(int t){ //#
    ...
    /*if (t == m){
        REP(i, m) G[i][j] = G[j][i] = 0;
    }
    else if (t == j-1){
        REP(i, m) G[i][j] = G[j][i] = G[i][j-1];
    }

    if (i && bj != t){
        REP(i, m) if (i == bj) G[i][j] = G[j][i] = 1;
    }*/

    if (j) G[j][j-1] = G[j-1][j] = 1;
    H[des].push();
...
void merge(){
    ...
    REP(i, m) G[j][i] = G[i][j] |= G[i][j-1];

    REP(i, m) if (b[i] == b[j-1]){
        REP(k, m) G[i][k] = G[k][i] = G[k][j];
        b[i] = b[j];
    }
    trans(b[j]);
}
...
void solve(){
    ...
    src = 0, des = 1; H[des].clear(); RST(b, G); d = 1; H[des].push();
    ...
    LL z = 0; REP(ii, H[des].sz) z += H[des].key[ii];
    cout << z << endl;
}
