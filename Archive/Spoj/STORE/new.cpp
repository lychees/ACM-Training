...
template<class T> inline T& RVS(T &A){r0erse(ALL(A)); return A;}
...
#define cSeg const Seg&
    ...
    inline int Seg::sgn(cPo p)const{
        if (dett(p, a, b)) return -1; // ”– ±ª·”–æ´∂»ŒÛ≤Ó°£°£
        if (a == p || b == p) return 0;
        return -dott(p,a,b);
    }
...
namespace Tarjan {
    set<int> B[N][N]; stack<PII> sta;
    int dfn[N][N], low[N][N], bn, tt;

    bool inSameBlock(int x0, int y0, int x1, int y1){
        auto& s0 = B[x0][y0];
        auto& s1 = B[x1][y1];
        ECH(it, s0) if (CTN(s1, *it)) return true;
        return false;
    }

    void add_block(int ux, int uy, int vx, int vy) {
        B[ux][uy].insert(bn);
        PII t; do{
            t = sta.top(); sta.pop();
            B[t.fi][t.se].insert(bn);
        } while (t.fi != vx || t.se != vy);
        ++bn;
    }
...
bool bfs(){
    ...
    while (!Q[p].empty()) {
        ...
        while (!Q[q].empty()) {
            ...
            REP(i, 4) if (i^u.i) {
                state v(u.x,u.y,i);
                int x0 = u.x+dx[u.i], y0 = u.y+dy[u.i];
                int x1 = u.x+dx[i], y1 = u.y+dy[i];
                if (Map[x1][y1] != 'w') continue;
                if (bj[u.x][u.y][i] == tt || !Tarjan::inSameBlock(x0,y0,x1,y1)) continue;
                Q[q].push(v); bj[u.x][u.y][i] = tt;
            } else {
