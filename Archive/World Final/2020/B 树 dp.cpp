...
const int N = int(2e4) + 9, M = int(4e6) + 9;
vector<uint> dp[N]; uint best[N], sign[N];
VI W; VII adj[N]; uint cost;
int n;
...
void dfs(int u, int p) {
    ...
    sign[u] = cost * SZ(adj[u]);

    int ww = 0; for (auto e: adj[u]) checkMax(ww, w);
    int up = SZ(W) - (lower_bound(W.rbegin(), W.rend(), ww) - W.rbegin());
    dp[u].resize(up);

    for (auto e: adj[u]) {
        if (v == p) {
            REP(i, up) dp[u][i] += W[i] - w;
        } else {
            dfs(v, u); sign[u] += best[v];
            REP(i, up) dp[u][i] += min(sign[v] + W[i] - w, i < SZ(dp[v]) ? dp[v][i] : UINT_MAX);
        }
    }

    best[u] = sign[u];
    for (auto d: dp[u]) checkMin(best[u], d);
}
#undef v
#undef w
