...
int main() {
...
#endif
    RD(n); REP(i, n) {
        char c; scanf(" %c%d", &c, &t[i]);
        VI &s = S[t[i]].s; if (s.empty()) s.PB(0);
        s.PB(s.back() + (c=='s'?1:-1));
    }

    int cnt = 0;
    for (auto& it: S) {
        VI &s = it.se.s; int &m = it.se.m;
        if (s.back() == 0) {
            m = *min_element(ALL(s));
            if (!m) ++cnt;
        }
    ...
    int best_cnt = cnt, best_pos = 0;

    REP(pos, n) {
        VI &s = S[t[pos]].s;
        if (s.back() == 0) {
            int &m = S[t[pos]].m, &i = S[t[pos]].i;
            if (s[i++] == m) --cnt;
            if (s[i] == m) {
                if (++cnt > best_cnt) {
                    best_cnt = cnt;
