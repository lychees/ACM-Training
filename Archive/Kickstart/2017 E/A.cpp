...
int f(){
    cin >> s; n = s.size();
    FLC(dp, 0x3f); dp[0][0][0] = 1;
    REP(i, n){
#define u dp[i][l][r]
        int t = INF; REP(l, i+1) FOR(r, l, i+1) checkMin(t, u);
        if (i == n-1) return t; ++t;
        REP(l, i+1) FOR(r, l, i+1){
            checkMin(u, t); checkMin(dp[i+1][l][r], u+1);
            if (match(l, r, i+1)){
                int ii = i+(r-l+1);
                checkMin(dp[ii][l][r], u+1);
            }
        }
    }
    return INF;
