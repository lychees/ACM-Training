...
void gauss() {
    REP(i, n) {
        int r = i; FOR(j, i+1, n) if (fabs(A[j][i]) > fabs(A[r][i])) r = j;
        assert(sgn(A[r][i]));

        if (i^r) {
            REP(j, n) swap(A[i][j], A[r][j]);
            swap(B[i], B[r]);
        }
        DB &t = A[i][i];
        FOR(j, i+1, n) A[i][j] /= t;
        ...
        REP(r, n) if (i^r) {
            DB &t = A[r][i]; if (!sgn(t)) continue;
            FOR(j, i+1, n) A[r][j] -= A[i][j] * t;
            B[r] -= B[i] * t; t = 0;
        }
    }
}
