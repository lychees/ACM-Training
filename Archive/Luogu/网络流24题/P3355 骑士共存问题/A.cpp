...
const int _N = int(2e2) + 9, N = int(4e4) + 9;
bool bid[_N][_N];
int n, bn;

struct Hopcroft_Karp {
    VI adj[N]; int hx[N], hy[N], px[N], py[N];
    int n;
...
#define z py[y]
    ...
    bool bfs(){
        static int Q[N]; bool res = 0; int l = 0, r = 0; RST(hx, hy);
        REP(x, n) if (px[x] == -1) Q[r++] = x;
        while (l < r) {
            int x = Q[l++];
            ECH(it, adj[x]) if (!hy[y]){
                hy[y] = hx[x] + 1; if (z == -1) res = 1;
                else hx[Q[r++] = z] = hy[y] + 1;
            }
        }
        return res;
    }
...
#undef z
    int gao() {
        int z = 0; FLC(px, py, -1); while(bfs()){
            REP(i, n) if (px[i] == -1 && dfs(i)) ++z;
        }
        return z;
    }
} H;
...
int main(){
    ...
    RD(n, bn); DO(bn) bid[RD()-1][RD()-1] = 1;
    REP_2(i, j, n, n) if (!bid[i][j] && ((i^j)&1)) {
        REP(d, 8) {
            int x = i + dx[d], y = j + dy[d];
            if (inBoard(x, y) && !bid[x][y]) {
                H.add_edge(get(X,{i,j}), get(Y,{x,y}));
            }
        }
    }
    H.n = SZ(X);

    cout << n*n - bn - H.gao() << endl;
}
