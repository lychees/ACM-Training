...
const int N = int(1e2) + 9, M = int(1e5) + 9;
...
int main(){
    ...
    RD(n, m); REP(i, n) RD(v[i],w[i],a[i]);
    
    int p = 0, q = 1; RST(dp[p]);
    REP(i, n) {
        swap(p, q); RST(dp[p]);
        REP(r, w[i]) {
            cz = 0, op = 0;
#define j (_j*w[i]+r)
#define f(k) (dp[q][(k)] - (k)/w[i]*v[i])
            for(int _j=0;j<=m;++_j){
                while (cz<op && f(j)>=f(Q[op-1])) --op;
                if (cz<op && (j-Q[cz] > a[i]*w[i])) ++cz;
                Q[op++] = j;
                dp[p][j] = j/w[i]*v[i]  + f(Q[cz]);
            }
        }
    }
