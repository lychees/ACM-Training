...
const int N = int(12), M = 1 << 20;
int A[N][N];
int n, m;

const int Prime = 9979; LL d; struct hashMap{
    ...
    void push(LL s){
        ...
        state[sz] = s; key[sz] = d;
        nxt[sz] = hd[x]; hd[x] = sz; ++sz;
        assert(sz < M);
    }
    void roll(int d){
        REP(ii, sz) state[ii] <<= d;
    }
    void display(){
        REP(ii, sz) cout << state[ii] << " " << key[ii] << endl;
        cout << endl;
    }
} H[2]; int src, des;
...
void init(){
    RST(A); n = 8; REP_2(i, j, n, m) A[i][j] = 1; Rush{
        char a, b; RC(a, b);
        A[b-'A'][a-'A'] = false;
    }
}
...
void solve(){
    ...
    src = 0, des = 1; d = 1; H[des].clear(); H[des].push(0);
    ...
    REP(i, n){
        REP(j, m){
            ...
            REP(ii, H[src].sz){
                LL s = H[src].state[ii]; d = H[src].key[ii];
                bool q1 = _1(s, 2*j), q2 = _1(s, 2*j+1), q3 = _1(s, 2*j+2);
                LL p1 = _1(2*j), p2 = _1(2*j+1), p3 = _1(2*j+2);
                bool o1 = (j+1-(i&1)) && A[i+1][j-(i&1)], o2 = A[i+1][j+1-(i&1)], o3 = A[i][j+1];
                ...
                int qq = q1 + q2 + q3; if (qq == 3) continue;

                if (qq == 2){
                    H[des].push(s);
                }
                else if (qq == 1){
                    if (o1) H[des].push(s^p1);
                    if (o2) H[des].push(s^p2);
                    if (o3) H[des].push(s^p3);
                }
                else{
                    if (o1 && o2) H[des].push(s^p1^p2);
                    if (o1 && o3) H[des].push(s^p1^p3);
                    if (o2 && o3) H[des].push(s^p2^p3);
                }
            }
        }
        if (!(i&1)) H[des].roll(2);
