...
int hd[N], to[M], prd[M], suc[M]; // Adjcent list ..
int dist[N], max_dist[N], sz[N], L[N], Ln, ans;
int n, k, _n, _c, c;
...
inline void dfs0(int u, int p = -1){
    max_dist[u] = dist[u];
    REP_G(i, u) if (v != p){
        dist[v] = dist[u] + 1;
        dfs0(v, u), checkMax(max_dist[u], max_dist[v]);
    }
}

inline void dfs1(int u, int p = -1){
    //cout << u << " " << p << endl;
    A[dist[u]] = cp(A[dist[u]].re + 1, 0);
    sz[u] = 1;
    REP_G(i, u) if (v != p){
        dfs1(v, u), sz[u] += sz[v];
    }
...
inline void find_center(int u, int p = -1){
    int blc = 0; sz[u] = 1;
    REP_G(i, u) if (v != p){
        find_center(v, u), sz[u] += sz[v];
        checkMax(blc, sz[v]);
    }
    checkMax(blc, _n - sz[u]);
...
inline void solve(int u = 0){
    ...
    _c = INF, find_center(u), u = c;
    dist[u] = 0, dfs0(u), nn = min(N, cover_bit(max_dist[u]*2+1)), fill(A, A+nn, 0), dfs1(u), stat();

    REP_G(i, u){
        nn = min(N, cover_bit(max_dist[v]*2+1)), fill(A, A+nn, 0), del(i^1), dfs2(v), stat(0);
        _n = sz[v], solve(v);
    }
}
...
int main(){
    ...
    FOR_C(i, 2, RD(n)<<1){
        RD(a, b);
        prd[hd[a]] = i, suc[i] = hd[a], hd[a] = i++;
        prd[hd[a]] = i, suc[i] = hd[a], hd[a] = i;
    }

    s = 0, _n = n, sieve(n), solve();
